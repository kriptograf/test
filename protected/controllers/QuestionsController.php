...
class QuestionsController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if (isset($_POST['question'])) 
                {
			$model->question=  CHtml::encode($_POST['question']);
                        $model->status = 0;
			if ($model->save()) 
                        {
                            $relModel = new QuestToRelation();
                            $relModel->id_quest = $_POST['id'];
                            $relModel->id_rel = $model->id;
                            $relModel->save();
                            $this->renderPartial('_newQuest', array('model'=>$model));
	...
	{
            $model = Questions::model()->published();
